...
#    RHS = T*K_g.dot(-1*dUf)
    ...
    for i in range(FemInf.Nnode):
        if(FemInf.Node_xy[i,2]>0.9999):
            F_output = F_output + Fn[2*i+1]

    print ("Dis = %f, Reaction Force Fy= %f \n" % (FemInf.U_tot[13*2+1], F_output))
    MacroInf.File_Force.write("%10.7f  %10.7f\n" % (FemInf.U_tot[13*2+1], F_output))
